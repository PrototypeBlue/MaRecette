<?php
 ...
 $sql="update orderdetails set order_status='Pending'  where nropedido=$id_factura";
               // echo $sql;
...
<?php
 ...
 include("config.php");
	...
	if(isset($_GET['delete_id']))
            ...
            $sql=" UPDATE items
                INNER JOIN
            orderdetails ON orderdetails.item_id= items.item_id and orderdetails.order_id=$idpedido SET 
            items.candisp=items.candisp+orderdetails.order_quantity,items.cantres=items.cantres-orderdetails.order_quantity";
             $resultado = $dbcon->query($sql);
             $sql="DELETE FROM orderdetails WHERE order_id =$idpedido";
             $resultado = $dbcon->query($sql);
             $dbcon->close();
          //  echo $sql;
...
<?php
	...
	if(isset($_GET['update_id']))
	{
        ...
        $resultado = $dbcon->query($sql);
            ...
            $fila = $resultado->fetch_assoc();
                ...
                $sql="update orderdetails set order_status='Ordered-Pend_Pago' , nropedido=$nrof WHERE order_status='Pending' and user_id =$user_id";
               // echo $sql;
                $resultado = $dbcon->query($sql);
            }
       $sql="select sum(order_total) as totalx from orderdetails where user_id=$user_id and nropedido=$nrof";
       $resultado = $dbcon->query($sql);
	...
	//	header("Location:Pagopp.php?concept=$dato_encriptadof&amt=$dato_encriptadom");
	}
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
         ...
         <center><h3> <span class="fa fa-cart-plus"></span> Listas de carro de compras</h3></center>
        </div>
...
include("config.php");
	...
	if($stmt->rowCount() > 0)
	{
		...
		echo "<td>";
		echo "<a class='btn btn-block btn-success' href='?update_id=".$user_id."' ><span class='glyphicon glyphicon-shopping-cart'></span> Ordene Ahora!</a>";
		echo "</td>";
